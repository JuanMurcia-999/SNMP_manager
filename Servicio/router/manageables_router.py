...
@router.get(
    ...
    dependencies=[Security(get_current_active_user, scopes=["feature:read:default"])],
...
@router.get(
    ...
    dependencies=[Security(get_current_active_user, scopes=["feature:read:active"])],
    response_model=list[schema.ReadDefaultFeature],
)
async def get_deafult_feature_agent(
    id: int, type: int, db: AsyncSession = Depends(get_db)
...
@router.post(
    "/exect-task/",
    dependencies=[Security(get_current_active_user, scopes=["task:exect"])],
...
@router.post(
    "/task/stop/",
    dependencies=[Security(get_current_active_user, scopes=["task:stop"])],
    status_code=status.HTTP_202_ACCEPTED,
)
async def stop_instance(request: schema.Manageable, db: AsyncSession = Depends(get_db)):
